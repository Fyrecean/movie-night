    ...
    return (
        <div>
            ...
            {recommendations.map((recommendation) => (
                    ...
                    {rsvped ? <div className="flex flex-row">
                            <VoteButton type='up' count={recommendation.upvotes} highlight={recommendation.myVote === 'up'} onClick={() => handleVote(recommendation.movie.id, 'up')} />
                            <VoteButton type='down' count={recommendation.downvotes} highlight={recommendation.myVote === 'down'} onClick={() => handleVote(recommendation.movie.id, 'down')} />
                        </div> : null}
                    </Movie>
            ))}
        </div>
...
const VoteButton = ({ type, count, highlight, onClick }: IVoteButtonProps) => {
    ...
    const classes = `${basecolor} py-1.5 px-3 hover:scale-105 hover:${color} hover:shadow text-center border border-gray-300 rounded-md border-gray-400 h-8 text-sm flex items-center gap-1 lg:gap-2`;
